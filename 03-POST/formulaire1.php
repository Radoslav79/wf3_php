...
// $_POST étant une superglobale, est donc un array et est disponible dans tous les contextes du script, y compris au sein des fonctions sans y faire "globale $_POST". 

// Les données saisies dans le formulairesont réceptionnées dans $_POST de la manière suivante : $_POST = array('name' => 'valeur input1', 'nameN' => 'valeur inputN');

echo '<pre>';
print_r($_POST); // pour vérifier que le formulaire envoie bien des données
echo '</pre>';

if (!empty($_POST)) { // si n'est pas vide $_POST c'est que l'on a reçu des données formulaire. On peut donc en afficher le contenu :
    echo '<p>Prénom : ' . $_POST['prenom'] . '</p>';
    echo '<p>Description : ' . $_POST['description'] . '</p>';
}

// Remarque : 
// - Cliquer dans l4URL et faire entrer permet de réinitialiser le formulaire comme si nous venions pour la prmeière fois.
...
<form method="post" action=""> <!-- un formulaire doit toujours être dans une balise <form> pour fonctionner. L'attribut method définit comment les données vont circuler entre le navigateur et le serveur. L'attribut action définit l'URL de destination des données saisies. -->
    ...
    <div><input type="text" name="prenom" id="prenom"></div> <!-- il ne faut pas oublier les "name" dans les formulaire : ils constituent les indices du tableua $_POST qui réceptionne les données -->
