...
if (!empty($_POST)) { // si $_POST n'est pas vide c'est que le formulaire a été envoyé
    ...
    if (!isset($_POST['pseudo']) || strlen($_POST['pseudo']) < 4 || strlen($_POST['pseudo']) > 20) { // si le pseudo n'existe pas dans $_POST c'est que le formulaire est altéré, ou si sa valeur est inférieur à 4 ou si sa valeaur est > 20 (pour la BDD), on met un message d'erreur à l'internaute.
    ...
    if (!isset($_POST['email']) || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) || strlen($_POST['email']) > 50) { // la fonction prédéfinie filter_var() avec l'argument FILTER VALIDATE_EMAIL vérifie que le string fourni est un email
    ...
    if (empty($contenu)) { // si la variable est vide c'est qu'il n'y a pas de message d'erreur
        ...
        // .... 
        // suite dans functions.php

        if ($resultat->rowCount() > 0) { //si la requête retourne une ou plusieurs lignes c'est que le pseudo est déjà en BDD
            $contenu .= '<div class="alert alert-danger">Le pseudo indisponible , veuillez en choisir un autre</div>';
        } else {
            // sinon on fait l'inscription en BDD
            $mdp = password_hash($_POST['mdp'], PASSWORD_DEFAULT); // Nous hachons le mot de passe avec l'algorithme bcript par défaut qui nous retourne une clé de hachage. Nous allons insérer en BDD
            // debug($mdp);
            $succes = executeRequete(
                "INSERT INTO membre (pseudo, mdp, nom, prenom, email, civilite, ville, code_postal, adresse, statut) VALUES (:pseudo, :mdp, :nom, :prenom, :email, :civilite, :ville, :code_postal, :adresse, :statut)      ",
                 array(':pseudo'    => $_POST['pseudo'], 
                        ...
                        ':statut' => 0, //0 pour un membre nom admin
                ));
