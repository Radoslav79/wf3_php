...
$resultat = executeRequete("SELECT DISTINCT categorie FROM produit"); // DISTINC pour dédoublenner les catégories et ne pas faire de répétition

$contenu_categories .= '<div class="list-group mb-4">';

    $contenu_categories .= '<a href="?categorie=tous" class="list-group-item-action">Tous les produits</a>'; // on passe dans l'URL que cette catégorie a pour valeur "tous" pour afficher tous les produits

    while ($cat = $resultat->fetch(PDO::FETCH_ASSOC)) {
        // debug($cat); // $cat est un tableau avec un indice "categorie" qui contient une catégorie différente par tour de bloucle.
        $contenu_categories .= '<a href="?categorie='. $cat['categorie'] .'" class="list-group-item-action">' . $cat['categorie'] . '</a>';
