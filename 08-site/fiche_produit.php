...
// 1- Contrôle de l'existance du produit demandé (un produit en favoris a pu être supprimé de la BDD...) :
// debug($_GET);
...
if (isset($_GET['id_produit'])) { // si "id_produit" est dans l'URL, c'est que l'on a demandé le détail d'un produit
    ...
    // 2- Affichage et mise ne forme des informations du produit :
    ...
    if ($stock > 0) { // s'il y a du stock sur le produit, on affiche le bouton
        ...
        for ($i = 1; $i <= $stock && $i <= 10; $i++) { // on fait 10 tours de boucles max à concurence du stock disponible (exemple : si le stock est de 3 on ne fait que 3 tours)
...
// Exercice : afficher 2 produits (photo et titre) aléatoirement appartenant à la catégorie du produit affiché au-dessus. Ces 2 produits doivent être différents du produit consulté. La photo est cliquable et mêne à la fiche détaillé du produit. Vous utilisez la variable $suggestion pour afficher le contenu.
...
<div class="row">
    <div class="col-12">
        <h1><?php echo $titre; // on accède à cette variabble grâce à l'extract fait sur le tableau $produit 
            ?></h1>
    </div>
    <div class="col-md-8">
