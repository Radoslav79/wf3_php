...
if (!estAdmin()) { // si membre nono admin ou non connecté, on le renvoie vers connexion.php
...
if (isset($_GET['id_produit'])) { // si existe id_produit dans l'URL c'est qu'on a demandé sa suppression
    ...
    if ($resultat) { //si le DELETE retourne un objet PDOStatement évalué à true, c'est que la requête a marché
...
$contenu .= '<p>Nombre de produits dans la boutique : ' . $resultat->rowCount() . '</p>';
    ...
    // Affichez un produit par ligne avec la colonne "photo" la photo du produit en 90px de large
    ...
    while ($produit = $resultat->fetch(PDO::FETCH_ASSOC)) { 
        ...
        $contenu .= '<td>
                        ...
                        <a href="gestion_boutique.php?id_produit='. $produit['id_produit'] .'" onclick="return confirm(\'Êtes-vous certain de vouloir supprimer ce produit ?\')" >supprimer</a>       
