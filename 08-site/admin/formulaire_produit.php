...
if (!estAdmin()) { // si membre nono admin ou non connecté, on le renvoie vers connexion.php. Renvoie true ou false
    header('location:../connexion.php'); // attention aux ../
    exit();
}
...
if (!empty($_POST)) { // si le formulaire a été envoyé
    ...
    if (isset($_POST['photo_actuelle'])) { // si existe "photo_actuelle" dans $_POST c'est que nous sommes en train de modifier le poduit avec sa photo. On remet l'URL de la photo en BDD :
    ...
    if (!empty($_FILES['photo']['name'])) { // si un fichier est en cours d'upload
        ...
        $photo_bdd = 'photo/' . $fichier_photo; // chemin relatif de la photo qui est enregistré en BDD et qui nous servira pour l'attribut "src" dans les balises images (les photos sont copiées dans le dossier "photo" ligne suivante).
...
if ($requete) { // si executeRequete a retourné un objet PDOStatement, donc implicitement évalué à true, alors c'est que la requête a marché
    $contenu .= '<div class="alert-success">Le produit a été enregistré.</div>';
...
if (isset($_GET['id_produit'])) { // si "id_produit" est dans l'URL, c'est que nous avons demandé la modification du produit : on sélectionne les infos du produit en BDD pour l'afficher dans le formulaire
    ...
    $produit = $resultat->fetch(PDO::FETCH_ASSOC); // on fetch le produit sans boucle while car il n'y en a qu'un seul par identifiant. 
...
<form action="" method="post" enctype="multipart/form-data"><!-- l'attribut enctype spécifie que le formulaire envoie des données binaires (fichier) et du texte (champs de formulaire) : permet d'uploader une photo -->
    ...
    <!-- 5- upload de photo -->
    <input type="file" name="photo" id="photo"><!-- on oublie pas l'attribut enctype="multipart/form-data" sur la balise <form> -->
