...
if (!empty($_POST)) { // si le formulaire d'ajout au panier a été envoyé
    ...
    // On remplit la session avec les infos du produit ajouté au panier (on les met dans la session pour que le panier soit accessible dans toutes les pagtes du site)
    ...
    $_SESSION['panier']['quantite'][] = $_POST['quantite']; // la quantité vient du formulaire donc de $_POST
    ...
    header('location:fiche_produit.php?id_produit=' . $_POST['id_produit']); // je passe l'id_produit dans l'URL pour réafficher la fiche de ce produit 
    exit();
...
if (empty($_SESSION['panier']['id_produit'])) { // S'il n'y a pas d'id_produit c'est que le panier est vide
